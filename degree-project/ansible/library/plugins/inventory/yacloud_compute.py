...
DOCUMENTATION = '''
    ...
    options:
        ...
        sa_key:
            description: File with service account keys
        yacloud_clouds:
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _init_client(self):
        ...
        if not token:
            raise AnsibleError("The token is empty, provide either `yacloud_token_file` or `yacloud_token`")
    ...
    def _process_hosts(self):
        ...
        for instance in self.hosts:
            for x in [x for x in instance["labels"] if group_label in x]:
                group = instance["labels"][x]
