...
Практические занятия были выполнены на Yandex.Cloud. Для извлечения учетных данных вне кода можно использовать отдельный, собранный  командой `yc iam key create --service-account-name my-robot --output key.json` и добавив его в исключение `.gitignore`. Данный пример показан в следующей задаче.

```BASH
$ yc config list
token: *******_**********************-**
cloud-id: b1gm04d20fit9ltelv1p
folder-id: b1gu9vr18o6v8qc41svt
...
1. Файлы конфигурации инфраструктуры [терраформа](./src/)
...
+----------------------+--------+---------------+---------+---------------+----------------+
| fhm30fnmaskk2gcq41v5 | node01 | ru-central1-a | RUNNING | 62.84.127.189 | 192.168.101.10 |
+----------------------+--------+---------------+---------+---------------+----------------+

$ ssh centos@62.84.127.189
The authenticity of host '62.84.127.189 (62.84.127.189)' can't be established.
ECDSA key fingerprint is SHA256:IFNxnQvXr5BXVRkUZKEOay1nOHOwSifh2c26xhfaZ9c.
